...
extern union {
	unsigned short		semvals[1];	/* set semaphore values */
	struct semid_ds		ds;		/* set permission values */
	struct sembuf		semops[1];	/* operation buffers */
} semtmp;
...
/*
**	semconv - Convert user supplied semid into a ptr to the 
**		associated semaphore header.  A ptr to a 
...
semctl()
{
	...
	case IPC_SET:
		if (up->u_uid != sp->sem_perm.uid && up->u_uid != sp->sem_perm.cuid
			 && !suser())
				return;
		if (copyin((caddr_t)uap->arg, &semtmp.ds, sizeof(semtmp.ds))) {
			up->u_error = EFAULT;
			return;
		}
	...
	case IPC_STAT:
		if (ipcaccess(&sp->sem_perm, SEM_R))
			return;
		if (copyout(sp, (caddr_t)uap->arg, sizeof(*sp))) {
			up->u_error = EFAULT;
			return;
		}
		break;
	...
	case GETALL:
		if (ipcaccess(&sp->sem_perm, SEM_R))
			return;
		iov.iov_base = (caddr_t)uap->arg;
		iov.iov_len = sizeof(p->semval) * sp->sem_nsems;
		uio.uio_iov = &iov;
		uio.uio_iovcnt = 1;
		uio.uio_offset = 0;
		...
		for(i = sp->sem_nsems, p = sp->sem_base;i--;p++) {
			up->u_error = uiomove((caddr_t)&p->semval, sizeof(p->semval),
				UIO_READ, &uio);
			if (up->u_error)
				return;
		}
	...
	case SETALL:
		if (ipcaccess(&sp->sem_perm, SEM_A))
			return;
		iov.iov_base = (caddr_t)uap->arg;
		iov.iov_len = (int)(sizeof(semtmp.semvals[0]) * sp->sem_nsems);
		uio.uio_iov = &iov;
		uio.uio_iovcnt = 1;
		uio.uio_offset = 0;
		uio.uio_seg = UIOSEG_USER;
		uio.uio_resid = (int)(sizeof(semtmp.semvals[0]) * sp->sem_nsems);
		up->u_error = uiomove((caddr_t)semtmp.semvals,
...
semexit()
{
	register struct sem_undo *up, *p;	/* undo struct ptrs */
	...
	for(i = up->un_cnt; i--; ) {
		if ((sp = semconv(up->un_ent[i].un_id)) == NULL)
			continue;
		v = (long)(semp = sp->sem_base +
		       up->un_ent[i].un_num)->semval + up->un_ent[i].un_aoe;
...
semget()
{
	...
	int semipcget();		/* routine to return perm info */

	if ((sp = (struct semid_ds *)
		ipcget(uap->key, uap->semflg, semipcget, &s)) == NULL)
			return;
	if (s) {
...
semop()
{
	...
	iov.iov_base = (caddr_t)uap->sops;
	iov.iov_len = (int)(uap->nsops * sizeof(*op));
	uio.uio_iov = &iov;
	uio.uio_iovcnt = 1;
	uio.uio_offset = 0;
	uio.uio_seg = UIOSEG_USER;
	uio.uio_resid = (int)(uap->nsops * sizeof(*op));
	u.u_error = uiomove((caddr_t)semtmp.semops, (int)(uap->nsops * sizeof(*op)),
		UIO_WRITE, &uio);
	if (u.u_error)
		return;
...
check:
	...
	if (again) {
		...
		uio.uio_seg = UIOSEG_USER;
		uio.uio_resid = (int)(uap->nsops * sizeof(*op));
		u.u_error = uiomove((caddr_t)semtmp.semops,
			(int)(uap->nsops * sizeof(*op)), UIO_WRITE, &uio);
