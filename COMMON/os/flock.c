/*	@(#)flock.c	UniPlus 2.1.1	*/

#include "sys/types.h"
#include "sys/param.h"
#	define	SLEEP(ptr, pri)		(caddr_t)sleep(ptr, pri)
...
struct file *fp;
{
	...
	register dev_t d;
	register ino_t n;
	struct inode *ip = ((struct inode *) ((struct vnode *) fp->f_data)->v_data);
...
struct file *fp;
{
	...
	if (flip != NULL) {
		...
		++flip->fl_refcnt;
		flip->fl_dev = ip->i_dev;
		flip->fl_number = ip->i_number;
	}
	return (flip);
}
...
register struct flock *lckdat;
{
	...
	if (fp->f_type != DTYPE_VNODE)
		...
		if (((struct vnode *) fp->f_data)->v_op != &svfs_vnodeops)
			return (EREMOTE);
		...
		case F_WRLCK:
			if ((found=blocked(flip->fl_flck, lckdat, &insrt)) == NULL)
				...
				if (deadflck(found)) {
					...
					if ((sf=insflck(&sleeplcks, lckdat, (struct filock *) NULL)) == NULL) {
						retval = ENOSPC;
					} else {
						found->stat.wakeflg++;
...
register int whence;
{
	register struct inode *ip = ((struct inode *) ((struct vnode *) fp->f_data)->v_data);
...
struct file *fp;
{
	...
	if ((fp->f_type == DTYPE_VNODE) &&
	    (((struct vnode *) fp->f_data)->v_op == &svfs_vnodeops)) {
		flip = findfid(fp);
		if (flip == NULL)
			return;
