/*	@(#)tcp_input.c 1.1 86/02/03 SMI; from UCB 6.2 83/11/04	*/
/*	@(#)tcp_input.c	2.1 86/04/15 NFSSRC */
...
trimthenstep6:
	 ...
	 * NOTE: strictly speaking, this violates the TCP protocol.
	 * RFC 793 says connections in a synchronized state (>= ESTABLISHED)
	 * should send an empty ack on unacceptable segments, however it
	 * also says that an RST should be sent if this side of the
	 * connection has "crashed".  We will be loose with the definition
	 * of crashed and say that if the process that opened the connection
	 * is gone (so_state & SS_NOFDREF) than the connection has crashed.
	 */
...
#define	ourfinisacked	(acked > 0)
		...
		/*
		 * If there are no more outstanding segments, turn off
		 * the retransmit timer, otherwise recalculate it.
		 */
		if (ti->ti_ack == tp->snd_max)
			tp->t_timer[TCPT_REXMT] = 0;
		else {
			TCPT_RANGESET(tp->t_timer[TCPT_REXMT],
			    (tcp_beta * tp->t_srtt) >> (FSHIFT*2),
			    TCPTV_MIN, TCPTV_MAX);
			tp->t_rxtshift = 0;
		}
...
dropwithreset:
	...
	if (tiflags & TH_ACK)
		...
		if (tiflags & TH_SYN)
			ti->ti_len++;
		tcp_respond(tp, ti, (tcp_seq)(ti->ti_seq+ti->ti_len),
