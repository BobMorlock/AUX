...
fill:	add.l	%d1, %d0			/* Now point to the end */
	add.l	&1, %d4
	mulu.l	&4, %d4				/* change for longs, to bytes */
	sub.l	&3, %d4				/* shrink the line length by the length of the borders */
	clr.l	%d5
...
jtk1:
		...
		clr.l	%d5
ll3:
		mov.b	&0xff, (%a0)+	/* Loop filling the scrolled */
		dbra	%d2, ll3		/*	region */
...
jtk4:
		...
		clr.l	%d5
ll4:
		clr.b	(%a0)+			/* Loop filling the scrolled */
		dbra	%d2, ll4		/*	region */
		/* mov.b	(%a0), (%a0)+		/* skip the right border */
		tst.l	%d5
		bne	jtk5
		mov.b	&0x1d, (%a0)+
		bra	jtk6
