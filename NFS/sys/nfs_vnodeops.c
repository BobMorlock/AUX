...
/*	@(#)nfs_vnodeops.c	1.3 of 12/6/86 Unisoft */
	...
	enum uio_rw rw;
{
	...
	if ((u.u_procp->p_compatflags & COMPAT_SYSCALLS) &&
	    rw == UIO_WRITE && vp->v_type == VREG &&
	...
	if (rw == UIO_WRITE && vp->v_type == VREG)
		if ((uio->uio_offset >> 9) >= u.u_limit)
			return (EFBIG);
		else
			if ((uio->uio_offset + uio->uio_resid) >> 9 > u.u_limit)
				uio->uio_resid = (u.u_limit - (uio->uio_offset >> 9)) << 9;
	size = vtoblksz(vp);
	size &= ~(DEV_BSIZE - 1);
	if (size <= 0) {
