/*	@(#)tcp_output.c 1.1 86/02/03 SMI; from UCB 6.2 83/11/04	*/
/*	@(#)tcp_output.c	2.1 86/04/15 NFSSRC */
...
/*
 * Size of "small" packet - try not to clog the net with these
 */
int	tcpsmall = 10;
...
again:
	...
	if (len) {
		if (len == tp->t_maxseg)
			goto send;
		/*
		 * Don't clog net with small packets
		 * See Oct. 84 SIGCOMM or nagle@ford
		 */
		if (off+len >= so->so_snd.sb_cc)
			if (off == 0 || off > tcpsmall || len > tcpsmall)
				goto send;
...
/*
 * Set pesist timer to cause resend of zero window probe.
 * Return boolean true if we have persisted TCP_MAXRXTSHIFT
 * times.
 */
int
tcp_setpersist(tp)
	register struct tcpcb *tp;
{
	...
	TCPT_RANGESET(tp->t_timer[TCPT_PERSIST],
	    ((tcp_beta * tp->t_srtt) >> (FSHIFT*2)) << tp->t_rxtshift,
